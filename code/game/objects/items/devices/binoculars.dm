...
/obj/item/device/binoculars/range/get_examine_text(mob/user)
	. = ..()
	. += SPAN_NOTICE(FONT_SIZE_LARGE("The rangefinder reads: LONGITUDE $1, LATITUDE $2, HEIGHT $3.", list(last_x, last_y, last_z))) // SS220 EDIT ADDICTION
...
/obj/item/device/binoculars/range/proc/show_coords(mob/user)
	if(rangefinder_popup)
		...
		to_chat(user, SPAN_NOTICE(FONT_SIZE_LARGE("SIMPLIFIED COORDINATES OF TARGET. LONGITUDE $1. LATITUDE $2.", list(last_x, last_y)))) // SS220 EDIT ADDICTION

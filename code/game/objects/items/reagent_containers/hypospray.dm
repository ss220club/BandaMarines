...
/obj/item/reagent_container/hypospray/attack(mob/living/M, mob/living/user)
	...
	if(skilllock == SKILL_MEDICAL_TRAINED && !skillcheck(user, SKILL_MEDICAL, SKILL_MEDICAL_TRAINED))
		user.visible_message(SPAN_WARNING("[capitalize(user.declent_ru(NOMINATIVE))] fumbles with [src]..."), SPAN_WARNING("You fumble with [src]..."))
	...
	if(M.reagents)
		...
		if(mag)
			to_chat(user, SPAN_NOTICE("Введено [trans] единиц. В [declent_ru(PREPOSITIONAL)] осталось [reagents.total_volume] единиц ([mag.name]).")) // SS220 EDIT ADDICTION
		else
			to_chat(user, SPAN_NOTICE("Введено [trans] единиц. В [declent_ru(PREPOSITIONAL)] осталось [reagents.total_volume] единиц.")) // SS220 EDIT ADDICTION

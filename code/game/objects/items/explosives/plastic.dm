...
/obj/item/explosive/plastic/afterattack(atom/target, mob/user, flag)
	...
	user.visible_message(SPAN_WARNING("[capitalize(user.declent_ru(NOMINATIVE))] пытается установить [ru_name] на [target]!"),
	SPAN_WARNING("Вы пытаетесь установить [ru_name] на [target]!"))
	if(ismob(target))
		var/mob/M = target
		to_chat(M, FONT_SIZE_HUGE(SPAN_DANGER("[capitalize(user.declent_ru(NOMINATIVE))] пытается установить [ru_name] на вас!"))) // SS220 EDIT ADDICTION
	...
	if(ismob(target))
		var/mob/M = target
		to_chat(M, FONT_SIZE_HUGE(SPAN_DANGER("[capitalize(user.declent_ru(NOMINATIVE))] устанавливает [ru_name] на вас!"))) // SS220 EDIT ADDICTION
	...
	if(customizable)
		user.visible_message(SPAN_WARNING("[capitalize(user.declent_ru(NOMINATIVE))] устанавливает [ru_name] на [target]!"),

...
/obj/item/get_examine_text(mob/user)
	...
	. += "Это [blood_color ? blood_color == COLOR_OIL ? "замасленн[genderize_ru(gender, "ый", "ая", "ое", "ые")] " : "окровавленн[genderize_ru(gender, "ый", "ая", "ое", "ые")] " : ""][icon2html(src, user)][declent_ru()]. Это предмет [size]." // SS220 EDIT ADDICTION
	if(desc)
		. += desc
	if(desc_lore)
...
/obj/item/proc/showoff(mob/user)
	...
	user.langchat_speech(SPAN_TRANSLATE("держит в руках [ru_name]"), viewers, GLOB.all_languages, skip_language_check = TRUE, animation_style = LANGCHAT_FAST_POP, additional_styles = list("langchat_small", "emote")) // SS220 EDIT ADDICTION

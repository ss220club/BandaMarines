...
/obj/structure/barricade/sandbags/attackby(obj/item/W, mob/user)
	...
	if(istype(W, stack_type))
		...
		user.visible_message(SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] starts adding more [SB] to [src]."),
		...
		for(var/i = build_stage to BARRICADE_SANDBAG_5)
			...
			increment_build_stage()
			update_icon()
		user.visible_message(SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] finishes stacking [SB] onto [src]."),

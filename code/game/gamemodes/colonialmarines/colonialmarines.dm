...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_LV1021_FALCONS_FURY)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(clf_announcement), "Бойцы, говорит координатор лагеря! Всем вооружиться! Чертовы жуки ползут из джунглей и только что уничтожили группу, которую я отправил на восток. Связь с нашими бойцами западнее реки потеряна. Бросайте всё и забирайте любое оружие, припасы, снаряжение, что сможете найти. Мы в осаде! Приготовьтесь защищать лагерь любой ценой!", "Координатор лагеря КОФ", 'sound/AI/commandreport.ogg'), 5 SECONDS)

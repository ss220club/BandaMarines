...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		if(MAP_TYRARGO_RIFT)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Дети мои. Великая битва бушует в этом мире, мире, наводненном воинствами для нашего захвата! Однако эти воинства сражаются с большой яростью. Я направил ваш субулей в эту область, вы создадите здесь мощный кордон, чтобы прикрыть наш западный фланг, пока другой субулей захватывает крепость на вашем востоке, которая заполнена тысячами воинств!\n\nЧтобы помочь вам, я отправил перед вами легион одноразовых дронов, они гораздо менее разумны, чем вы, но их будет достаточно, чтобы заманить в засаду любые оставшиеся враждебные воинства на вашем западе, пока вы не обезопасите себя.", "everything", QUEEN_MOTHER_ANNOUNCE), 20 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Алмайер, это место эвакуации гражданского населения музея Тирарго. Нас атакует кластер XX-121, но мы держимся.\n\nУ нас сильные волны XX-121, приближающиеся с северо-востока, и мы находимся под сильным подавлением, наши эвакуационные суда прижаты дальними ударами Бойлеров, а западные выходы из города слишком опасны для движения наземных эвакуационных машин, мы просим вас обеспечить западный подход, чтобы вы могли подавить силы противника и обеспечить эвакуацию гражданского населения, прием.", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 15 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Будьте начеку, дети мои. Я чувствую, что грязные канализации этого так называемого города могут быть затоплены хозяевами в любой момент. Кнопка для спуска этой гнилой воды находится в металлической конструкции, которую хозяева называют очистными сооружениями канализации, хотя кнопка сработает только в том случае, если хозяева смогут восстановить подачу питания на «ЛКП», который находится на подстанции распределения электроэнергии, которая находится к востоку от канализационной станции.", "everything", QUEEN_MOTHER_ANNOUNCE), 15 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Внимание: Анализ планов городской планировки выявил возможное тактическое преимущество. Выпускной клапан может быть активирован на городской станции очистки канализации, этот клапан затопит нижние канализационные туннели водой, вытесняя значительное количество ксенобиологического роста.\n\nОднако этот клапан должен быть запитан путем ремонта специального ЛКП, расположенного на подземной электроподстанции, расположенной к востоку от подземной станции очистки канализации.", "ARES 3.2 Strategic Notice", 'sound/AI/commandreport.ogg'), 20 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Алмайер. Мы видим возросшую активность XX-121 в месте эвакуации Тирарго. Дополнительные штаммы приближаются с севера.\n\nВражеские бойлеры приблизились достаточно близко, чтобы подавить нашу поддержку с воздуха, мы переориентируем танки Longstreet, чтобы прикрыть наши фланги. Запрашиваю немедленное подавление вражеских сил вблизи нашего местоположения через западный вход в город, прием. ", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 35 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Все элементы, больше кластеров XX-121 вторгаются с нашего востока. Мы подвергаемся сильному нападению со всех сторон и потеряли половину нашей танковой поддержки Longstreet из-за Крашеров.\n\nМы исчерпали наши боеприпасы HEAP и были вынуждены перейти на боеприпасы с мягкой головной частью. Мы не можем терпеть это долго, запрашиваем срочную поддержку от сил Алмайер, прием.", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 60 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Это Тирарго. Ксеносы начали наступление с нашего южного фланга. У нас остался только один танк. Мы отступаем в средний коридор и переместили гражданских на внутренний периметр.\n\nСитуация ужасная, мы теряем силы. Нам нужна эта поддержка, прием.", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 80 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Всем элементам! Это место эвакуации Тирарго, наша ситуация критическая. Жуки окружили нас со всех сторон, наша бронетанковая поддержка уничтожена, и теперь нас прижимают к земле вражеские Опустошители.\n\nНам нужна срочная огневая поддержка, мы не можем больше терпеть.", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 100 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "Алмайер! Жуки хлынули во внутренний периметр! Гражданские берутся за оружие, чтобы защитить объект, но они необучены.\n\nНас одолевают, нам нужна огневая поддержка прямо сейчас! Сейчас же, черт возьми!", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 120 MINUTES)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "##&@* все мертвы! Тирарго захвачен! T&^@%###--- командный пункт в любую секунду, %$#* мы н#@##ы--------------------", "Tyrargo Civilian Evac, 1st Air Cav Headquarters", 'sound/AI/commandreport.ogg'), 140 MINUTES)

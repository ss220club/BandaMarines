...
if(MAP_THE_LAST_BUNKER)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "Внимание: Подъезжаем к бункеру K12. Пожалуйста, отойдите от двери.", "Train Automated Announcement", 'sound/AI/commandreport.ogg'), 1 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "Эй, ЭЙ! Ребята, вы в поезде. Вы вляпались по-серьезному, вам нужно выбираться отсюда немедленно! Поезд уже закрыт, и у вас есть всего две минуты до того, как ксеноморфы ворвутся в командный бункер, вы слышали меня, ксеноморфы! Найдите припасы, закопайте лаз и ждите спасения.\п\П Арсенал в западном бункере не тронут, а в северном бункере много металла.\п\Я пытаюсь снять внешнюю изоляцию, но это займет у меня еще десять минут. Больше я ничего не могу для вас сделать, удачи.", "Unknown Source", 'sound/AI/commandreport.ogg'), 10 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Победа! Обитатели этого металлического бункера были убиты, однако мы сильно пострадали от резни и должны оправиться. Я чувствую, что прибытие новой группы хозяев, они, пополнит наши ряды.\п\п Похоже, что бункер закрыт, но это неважно, я отправил дрона поменьше пожертвовать собой, чтобы снять блокаду, однако на это уйдет две минуты. Используйте это время, чтобы подготовиться.", "everything", QUEEN_MOTHER_ANNOUNCE), 20 SECONDS)

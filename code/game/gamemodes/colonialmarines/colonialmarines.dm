...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_THE_LAST_BUNKER)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Победа! Обитатели этого металлического бункера были убиты, однако мы сильно пострадали от резни и должны оправиться. Я чувствую, что прибытие новой группы хозяев, они, пополнит наши ряды.\n\n Похоже, что в бункере введен режим карантина, но это неважно, я отправил дрона поменьше пожертвовать собой, чтобы снять карантин, однако на это уйдет две минуты. Используйте это время, чтобы подготовиться.", "everything", QUEEN_MOTHER_ANNOUNCE), 20 SECONDS)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "Если кто-то еще жив, я снимаю внешнюю изоляцию. Я оборудовал укрепленную позицию прямо к западу от главного входа, найдите меня, я установил сторожевые орудия, убедитесь, что на вас есть опознавательные знаки... Подождите, а где же мои, ох бля...", "Истичник неизвестен", 'sound/AI/commandreport.ogg'), 4.5 MINUTES)

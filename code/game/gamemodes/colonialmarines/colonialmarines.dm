...
/datum/game_mode/colonialmarines/proc/lore_announcement()
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		...
		if(MAP_THE_LAST_BUNKER)
			...
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "Предупреждение: Обнаружен сбой блокировки команды", "Автоматизированная передача сигнала", 'sound/AI/commandreport.ogg'), 100 SECONDS)
			addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(venir_announcement), "Если кто-то еще жив, я снимаю внешнюю изоляцию. Я оборудовал укрепленную позицию прямо к западу от главного входа, найдите меня, я установил сторожевые орудия, убедитесь, что на вас есть опознавательные знаки... Подождите, а где же мои, ох бля...", "Источник неизвестен", 'sound/AI/commandreport.ogg'), 4.5 MINUTES)

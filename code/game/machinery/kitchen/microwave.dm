	...
	if(broken > 0)
		if(broken == 2 && HAS_TRAIT(O, TRAIT_TOOL_SCREWDRIVER)) // If it's broken and they're using a screwdriver
			user.visible_message(
				SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] starts to fix part of the microwave."),
				SPAN_NOTICE("You start to fix part of the microwave.")
			)
			if (do_after(user,20, INTERRUPT_ALL|BEHAVIOR_IMMOBILE, BUSY_ICON_BUILD))
				user.visible_message(
					SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] fixes part of the microwave."),
			...
			if (do_after(user,20, INTERRUPT_ALL|BEHAVIOR_IMMOBILE, BUSY_ICON_BUILD))
				user.visible_message(
					SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] fixes the microwave."),
		...
		if(istype(O, /obj/item/reagent_container/spray/cleaner)) // If they're trying to clean it then let them
			user.visible_message(
				SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] starts to clean the microwave."),
			...
			if (do_after(user, 2 SECONDS * user.get_skill_duration_multiplier(SKILL_DOMESTIC), INTERRUPT_ALL, BUSY_ICON_FRIENDLY))
				user.visible_message(
					SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] has cleaned the microwave."),
		...
		if(istype(O, /obj/item/stack) && O:get_amount() > 1) // This is bad, but I can't think of how to change it
			...
			S.use(1)
			user.visible_message(
				SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] has added one of [O] to \the [src]."),
		...
		// user.before_take_item(O) //This just causes problems so far as I can tell. -Pete
			if(user.drop_held_item())
				O.forceMove(src)
				user.visible_message(
					SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] has added \the [O] to \the [src]."),

...
/obj/structure/machinery/nuclearbomb/proc/announce_to_players(timer_warning)
	...
	if(timer_warning) //we check for timer warnings first
		...
		elder_overseer_message("The human purification device has been activated. You have approximately $1 to abandon the hunting grounds before it detonated.", attr = list(t_left)) // SS220 EDIT ADDICTION
		//xenos part
		var/warning
	...
	if(timing)
		...
		var/t_left = duration2text_sec(floor(rand(timeleft - timeleft / 10, timeleft + timeleft / 10)))
		elder_overseer_message("The human purification device has been deployed. You have approximately $1 to abandon the hunting grounds before it initialize decryption.", attr = list(t_left)) // SS220 EDIT ADDICTION
		for(var/hivenumber in GLOB.hive_datum)
			hive = GLOB.hive_datum[hivenumber]
			if(!length(hive.totalXenos))
				continue
			xeno_announcement(SPAN_XENOANNOUNCE("The tallhosts have deployed a hive killer at $1! Stop it at all costs!", list(get_area_name(loc))), hive.hivenumber, XENO_GENERAL_ANNOUNCE) // SS220 EDIT ADDICTION
	else
		...
		elder_overseer_message("The human purification device's signature has disappeared.")
		for(var/hivenumber in GLOB.hive_datum)
			hive = GLOB.hive_datum[hivenumber]
			if(!length(hive.totalXenos))
				continue
...
/obj/structure/machinery/nuclearbomb/tech/announce_to_players(timer_warning)
	...
	if(timer_warning)
		if(timer_warning == NUKE_DECRYPT_SHOW_TIMER_COMPLETE)
			...
			var/t_left = duration2text_sec(floor(rand(timeleft - timeleft / 10, timeleft + timeleft / 10)))
			elder_overseer_message("The human purification device has been decrypted. You have approximately $1 to abandon the hunting grounds before it activated.", attr = list(t_left)) // SS220 EDIT ADDICTION
			var/datum/hive_status/hive
			for(var/hivenumber in GLOB.hive_datum)
				hive = GLOB.hive_datum[hivenumber]
				if(!length(hive.totalXenos))
					continue
				xeno_announcement(SPAN_XENOANNOUNCE("The tallhosts have decrypted the hive killer at $1! Stop it at all costs!", list(get_area_name(loc))), hive.hivenumber, XENO_GENERAL_ANNOUNCE) // SS220 EDIT ADDICTION
			return
		...
		elder_overseer_message("You have approximately $1 seconds to abandon the hunting grounds before activation of the human purification device.", attr = list(time_left)) // SS220 EDIT ADDICTION
	...
	if(decrypting)
		...
		var/time_left = duration2text_sec(floor(rand(decryption_time - decryption_time / 10, decryption_time + decryption_time / 10)))
		elder_overseer_message("The human purification device has been initialize decryption. You have approximately $1 to abandon the hunting grounds before it decrypts.", attr = list(time_left)) // SS220 EDIT ADDICTION
		for(var/hivenumber in GLOB.hive_datum)
			hive = GLOB.hive_datum[hivenumber]
			if(!length(hive.totalXenos))
				continue
			xeno_announcement(SPAN_XENOANNOUNCE("The tallhosts have started the initial decrypt phase of a hive killer at $1! You have about $2 to destroy at least one of their communications relays!", list(get_area_name(loc), time_left)), hive.hivenumber, XENO_GENERAL_ANNOUNCE) // SS220 EDIT ADDICTION
		return

...
/obj/structure/machinery/computer/ares_console/proc/message_ares(text, mob/Sender, ref, fake = FALSE)
	...
	if(fake)
		log_say("[key_name(Sender)] faked the message '[text]' from [last_login] in ARES 1:1. (AREA: [log_location(Sender)])", Sender) //BANDAMARINES LOGIS EDIT
		msg = SPAN_STAFF_IC("<b><font color=orange>ARES:</font> [key_name(Sender, 1)] faked a message from '[last_login]':</b> [text]")
	else
		log_say("[key_name(Sender)] sent '[text]' to ARES 1:1. (AREA: [log_location(Sender)])", Sender) //BANDAMARINES LOGIS EDIT

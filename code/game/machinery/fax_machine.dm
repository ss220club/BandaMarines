...
/obj/structure/machinery/faxmachine/proc/outgoing_fax_message(mob/user, sending_priority)
	...
	log_game("FAX: [logis_identity(user)] sent '[original_fax.name]' from [identity_name] ([department]) to [the_target_department] via [network] at [log_location(src)]. Sender location: [log_location(user)].") //BANDAMARINES ADDITION

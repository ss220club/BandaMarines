	...
	if(!(flags_startup_parameters & ROLE_NO_ACCOUNT))
		...
		if(card && !user_has_preexisting_account)
			...
			if(account_user.mind)
				var/remembered_info = ""
				remembered_info += "<b>Ваш номер аккаунта:</b> #[generated_account.account_number]<br>"	// SS220 EDIT - Translate
				remembered_info += "<b>Пинкод вашего аккаунта:</b> [generated_account.remote_access_pin]<br>"	// SS220 EDIT - Translate
				remembered_info += "<b>Средств на вашем счету:</b> $[generated_account.money]<br>"	// SS220 EDIT - Translate

				if(length(generated_account.transaction_log))
					var/datum/transaction/T = generated_account.transaction_log[1]
					remembered_info += "<b>Ваш аккаунт был создан:</b> [T.time], [T.date] at [T.source_terminal]<br>"	// SS220 EDIT - Translate

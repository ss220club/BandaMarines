...
/proc/message_alien_candidates(list/candidates, dequeued, cache_only=FALSE)
	for(var/i in (1 + dequeued) to length(candidates))
		...
		var/cached_message = "Вы сейчас [i-dequeued]\th в очереди на становление личинкой." // SS220 EDIT ADDICTION
		...
		for(var/mob_name in cur_hive.banished_ckeys)
			if(cur_hive.banished_ckeys[mob_name] == candidate_observer.ckey)
				candidate_observer.larva_pool_cached_message += "\nВНИМАНИЕ: Вы изгнаны из улья [cur_hive] и не можете вернуться, \
					пока Королева не примет вас обратно или не умрёт. Ваш номер в очереди на становление личинкой не обновится, пока не появится улья, из которого вас не изгнали."
				break

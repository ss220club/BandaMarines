...
/datum/weather_ss_map_holder/proc/weather_warning(event_type)
	...
	for(var/mob/living/carbon/human/affected_human in GLOB.alive_human_list)
		if(!affected_human.stat && affected_human.client && (affected_human.z in ground_levels))
			...
			affected_human.play_screen_text("<span class='langchat' style=font-size:12pt;text-align:center valign='top'><u>Weather Alert:</u></span><br>" + "Incoming [weather_name]", /atom/movable/screen/text/screen_text/command_order, rgb(103, 214, 146)) // SS220 EDIT: font
	for(var/mob/living/carbon/xenomorph/affected_xeno in GLOB.living_xeno_list)
		if(!affected_xeno.stat && affected_xeno.client)
			...
			affected_xeno.play_screen_text("<span class='langchat' style=font-size:12pt;text-align:center valign='top'><u>The Hivemind Senses:</u></span><br>" + "Incoming [weather_name]", /atom/movable/screen/text/screen_text/command_order, rgb(175, 0, 175)) // SS220 EDIT: font

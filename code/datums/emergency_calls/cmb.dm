...
/datum/emergency_call/cmb/print_backstory(mob/living/carbon/human/M)
	if(M == leader)
		...
		to_chat(M, SPAN_BOLD("You are [pick(40; "skeptical", 30;"amicable", 30;"supportive")] of the USCM's actions on the frontier."))
...
/datum/emergency_call/cmb/riot_control/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(mob?.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(mob.client, JOB_SQUAD_LEADER, time_required_for_job))
		...
		arm_equipment(mob, /datum/equipment_preset/synth/cmb/riot, TRUE, TRUE)
	else

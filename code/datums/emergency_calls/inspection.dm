...
/datum/emergency_call/inspection_provost/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, list(JOB_WARDEN, JOB_CHIEF_POLICE), time_required_for_job))
		...
		to_chat(H, SPAN_ROLE_BODY("Вы направляетесь на борт [MAIN_SHIP_NAME] для расследования нераскрытого инцидента, связанного с несоблюдением Военного закона. Военная прокуратура может предоставить дополнительные сведения, однако вам следует немедленно проследовать в бриг и оценить обстановку."))

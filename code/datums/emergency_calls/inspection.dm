...
/datum/emergency_call/inspection_provost/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, list(JOB_WARDEN, JOB_CHIEF_POLICE), time_required_for_job))
		...
		to_chat(H, SPAN_ROLE_BODY("Вы направляетесь на борт [MAIN_SHIP_NAME] для расследования нераскрытого дела, связанного с несоблюдением Военного Закона. Военная прокуратура может предоставить дополнительные сведения, однако вам следует немедленно проследовать в бриг и оценить обстановку."))
		...
		to_chat(H, SPAN_WARNING("Эта роль требует знания Военного Закона и Стандартных Операционных Процедур. Обратитесь в Adminhelp, если у вас возникли вопросы или вы хотите уступить эту роль другому игроку."))
	else
...
/datum/emergency_call/inspection_wy
	...
	max_heavies = 1
	probability = 0
	ert_message = "Запрошен следователь ЧВК «Вейланд-Ютани» для расследования действий связного корпорации." //SS220 EDIT
...
/datum/emergency_call/inspection_wy/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, list(JOB_SQUAD_LEADER), time_required_for_job))
		...
		to_chat(H, SPAN_ROLE_HEADER("Вы - специалист по подавлению массовых беспорядков ЧВК «Вейланд-Ютани»!"))
...
/datum/emergency_call/inspection_wy/lawyer/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, list(JOB_SQUAD_LEADER), time_required_for_job))
		...
		to_chat(H, SPAN_ROLE_BODY("Помните, персонал ККМП на борту может негативно воспринять ваше присутствие. Вы и старший правовой представитель обязаны избегать открытого противостояния с морской пехотой. Ваша главная задача — обеспечить ваше выживание и подготовку отчёта, который требуется компании."))
...
/datum/emergency_call/inspection_cmb/black_market/New()
	...
	objectives = "Расследуйте несоответствия в ASRS корабля [MAIN_SHIP_NAME]. В случае незаконной деятельности соберите доказательства и подайте рапорт командованию КМБ на станции Анкорпойнт. При необходимости представитель комиссии по межзвёздной торговле может исправить поломку ASRS с помощью Tradeband Compliance Device. Сотрудничайте с военной полицией [MAIN_SHIP_NAME]." //SS220 EDIT

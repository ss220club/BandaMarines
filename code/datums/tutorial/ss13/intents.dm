...
/datum/tutorial/ss13/intents/start_tutorial(mob/starting_mob)
	...
	init_mob()
	message_to_player("Это обучение по <b>интентам</b> в Космической Станции 13. Выделенный UI элемент справа снизу это ваш текущий интент.")
...
/datum/tutorial/ss13/intents/proc/require_help()
	...
	message_to_player("Ваш интент был изменён с <b>помощи</b>. Поменяйте его обратно, нажав <b>[retrieve_bind("select_help_intent")]</b>.")
	update_objective("Верните интент помощи нажав [retrieve_bind("select_help_intent")].")
...
/datum/tutorial/ss13/intents/proc/on_help_intent(datum/source, new_intent)
	...
	message_to_player("Первый из интентов это интент <b>помощи</b>. Он используется что бы без вреда трогать других, тушить людей, давать исскуственное дыхание, и похожее. Нажмите на <b>Тестовую Куклу</b>, что бы похлопать её по спине.")
	update_objective("Нажмите на куклу в интенте помощи.")
...
/datum/tutorial/ss13/intents/proc/on_help_attack(datum/source, mob/living/carbon/human/attacked_mob)
	...
	message_to_player("Второй интент это <b>обезоруживание</b>, который можно поставить с <b>[retrieve_bind("select_disarm_intent")]</b>. Обезоруживание используется в основном, что бы толкать людей, из-за чего они могут выронить то что носили или упасть. Толкайте <b>Тестовую куклу</b> пока она не упадёт.")
	update_objective("Поменяйте свой интент на обезоруживание при помощи [retrieve_bind("select_disarm_intent")] и повалите куклу на землю.")
...
/datum/tutorial/ss13/intents/proc/on_shove_down(datum/source, datum/status_effect/new_effect)
	...
	message_to_player("Третий интент это <b>захват</b>. Захват используется для того, что бы взять людей спокойно, агресивно, или удушающе. Хватайте по нескольку раз, что бы \"обновить\" свой захват. Агресивно возьмите <b>Тестовую Куклу</b>.")
	update_objective("Агресивно возьмите куклу 2 раза нажав по ней.")
...
/datum/tutorial/ss13/intents/proc/on_aggrograb(datum/source, mob/living/carbon/human/choker)
	...
	message_to_player("Последний интент это <b>вред</b>. Вред используется что бы бить людей своим оружием или руками. Ударьте <b>Тестовую Куклу</b> пустой рукой.")
	update_objective("Атакуйте куклу пустой рукой.")
...
/datum/tutorial/ss13/intents/proc/on_harm_attack(datum/source, mob/living/carbon/human/attacked_mob)
	...
	message_to_player("Отлично. Это все основы которые вам надо знать. Это обучение закончится скоро.")
	update_objective("")

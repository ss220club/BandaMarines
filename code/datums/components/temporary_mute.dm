...
/datum/component/temporary_mute/proc/on_speak(
	...
	if(!nolog)
		msg_admin_niche("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to say the following before their spawn mute ended: [message] (CKEY: [user.cckey]) (JOB: [user.job]) (AREA: [log_location(user)])") //BANDAMARINES LOGIS EDIT
	if(on_speak_message)
...
/datum/component/temporary_mute/proc/on_hivemind(mob/user, message)
	...
	msg_admin_niche("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to hivemind the following before their spawn mute ended: [message] (CKEY: [user.cckey]) (JOB: [user.job]) (AREA: [log_location(user)])") //BANDAMARINES LOGIS EDIT
	if(on_speak_message)
		to_chat(parent, SPAN_BOLDNOTICE(on_speak_message))
...
/datum/component/temporary_mute/proc/on_emote(mob/user, datum/emote/current_emote, act, m_type, param, intentional)
	...
	msg_admin_niche("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to emote the following before their spawn mute ended: [param] (CKEY: [user.cckey]) (JOB: [user.job]) (AREA: [log_location(user)])") //BANDAMARINES LOGIS EDIT
	if(on_emote_message)
...
/datum/component/temporary_mute/proc/on_point(mob/user, atom/target)
	...
	msg_admin_niche("[user.name != "Unknown" ? user.name : "([user.real_name])"] attempted to point at the following before their spawn mute ended: [target] (CKEY: [user.cckey]) (JOB: [user.job]) (AREA: [log_location(user)])") //BANDAMARINES LOGIS EDIT
	if(on_emote_message)
		to_chat(parent, SPAN_BOLDNOTICE(on_emote_message))

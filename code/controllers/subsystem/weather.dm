...
/datum/controller/subsystem/weather/proc/end_weather_event()
	...
	if (weather_event_instance.display_name)
		message_admins(SPAN_BLUE("Weather Event of type $1 ending after $2.", list (weather_event_instance.display_name, DisplayTimeText(world.time - current_event_start_time)))) // SS220 EDIT ADDICTION
	else
		message_admins(SPAN_BLUE("Weather Event of unknown type $1 ending after $2.", list(weather_event_type, DisplayTimeText(world.time - current_event_start_time)))) // SS220 EDIT ADDICTION

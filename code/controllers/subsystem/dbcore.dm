...
/datum/controller/subsystem/dbcore/Shutdown()
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/query_round_shutdown = SSdbcore.NewQuery(
			...
			list("end_state" = "end", "round_id" = GLOB.round_id),
			TRUE
		)
...
/datum/controller/subsystem/dbcore/proc/Connect()
	...
	var/user = CONFIG_GET(string/db_username)
	var/pass = CONFIG_GET(string/db_password)
	var/db = CONFIG_GET(string/db_database)
	var/address = CONFIG_GET(string/db_address)
	var/port = CONFIG_GET(number/db_port)
...
/datum/controller/subsystem/dbcore/proc/SetRoundEnd()
	...
	var/datum/db_query/query_round_end = SSdbcore.NewQuery(
		...
		list("game_mode_result" = "finish", "station_name" = "BandaMarines", "round_id" = GLOB.round_id)
	)
...
/datum/db_query/proc/slow_query_check()
	message_admins("HEY! A database query timed out. Did the server just hang? <a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];slowquery=yes'>\[YES\]</a>|<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];slowquery=no'>\[NO\]</a>")

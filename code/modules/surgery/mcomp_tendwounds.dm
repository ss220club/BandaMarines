...
/datum/surgery_step/mstabilize_wounds/success(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, tool_type, datum/surgery/surgery)
	...
	if(user == target)
		...
		user.affected_message(target,
			...
			SPAN_HELPFUL("[capitalize(user.declent_ru(NOMINATIVE))] finishes stabilizing your wounds with [tool]."),
			SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] finishes stabilizing [target]'s wounds with [tool]."))
...
/datum/surgery_step/mtend_wounds
	...
	time = 15 SECONDS
	preop_sound = 'sound/effects/gunrustle1.ogg'
	failure_sound = 'sound/surgery/organ2.ogg'
...
/datum/surgery_step/cauterize/mclamp_wound/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, tool_type, datum/surgery/surgery)
	if(user == target)
		...
		user.affected_message(target,
			...
			SPAN_HELPFUL("[capitalize(user.declent_ru(NOMINATIVE))] begins to close the treated wounds on your body with [tool]."),
			SPAN_HELPFUL("[capitalize(user.declent_ru(NOMINATIVE))] begins to close the treated wounds on [target]'s body with [tool]."))
...
/datum/surgery_step/cauterize/mclamp_wound/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, tool_type, datum/surgery/surgery)
	...
	if(user == target)
		user.visible_message(SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] finishes closing the treated wounds on their body with [tool]."),
		...
		user.affected_message(target,
			...
			SPAN_HELPFUL("[capitalize(user.declent_ru(NOMINATIVE))] finishes closing your treated wounds with [tool]."),
			SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] finishes closing [target]'s treated wounds with [tool]."))

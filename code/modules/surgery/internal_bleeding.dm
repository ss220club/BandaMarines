...
/datum/surgery_step/fix_vein/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, tool_type, datum/surgery/surgery)
	user.affected_message(target,
		...
		SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] starts to patch the damaged vein in your [surgery.affected_limb.display_name] with [tool]."),
		SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] starts to patch the damaged vein in [target]'s [surgery.affected_limb.display_name] with [tool]."))

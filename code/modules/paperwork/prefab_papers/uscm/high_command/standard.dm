/datum/prefab_document/uscm/highcom/standard/New()
	...
	dat += "<p class=\"header-title\">Сообщение Верховного Командования USCM</p>"
	...
	dat += "<p>"
	dat += "Это сообщение и любые файлы, переданные с ним, являются конфиденциальными и предназначены исключительно для использования лицом или организацией, к которым они адресованы. Если вы не являетесь вышеупомянутым адресатом, вы не должны распространять, дистрибьютировать или копировать это послание.<br>"
	dat += "Если вы не является предполагаемым получателем, вам сообщается, что раскрытие, копирование, распространение или принятие каких-либо действий в зависимости от содержания этой информации строго запрещены.<br>"
	dat += "</p>"

/datum/prefab_document/wey_yu/highcom/standard/New()
	...
	dat += "<p>Индекс: <span class=\"paper_field\"></span><br>"
	...
	dat += "Тема: <span class=\"paper_field\"></span></p>"
	dat += "<p><ul><span class=\"paper_field\"></span></ul></p>"
	dat += "<HR>"
	dat += "<p>Имя отправителя: <span class=\"paper_field\"></span><br>"
	dat += "Подпись: <span class=\"paper_field\"></span></p>"
	dat += "</div> <!-- /message-body-text -->"
	dat += ""
	dat += "<div class='message-signature-text'>"
	dat += "<p class='disclaimer'>Это сообщение и любые файлы, переданные с ним, являются конфиденциальными и предназначены исключительно для использования индивидуумом или организацией, к которым они адресованы. Это сообщение содержит конфиденциальную информацию и предназначено только для адресата, если вы не указанный адресат, вы не должны распространять, распределять или копировать этот документ. Если вы не являетесь предполагаемым получателем, вам сообщается, что раскрытие, копирование, распределение или принятие любых действий, основываясь на содержании этой информации, строго запрещено.</p>"

/datum/prefab_document/provost/highcom/dao_response/New()
	...
	dat += "<b>Ответственный сотрудник:</b> <span class=\"paper_field\"></span><br>"
	...
	dat += "Запрос на ЗПД требует, чтобы ответчик был освобожден из-под стражи, раздел 'Был запрошен PR301b' на связанной форме PR301a был заполнен, и эта форма была отправлена по факсу в офис Провоста.<br>"
	dat += "После получения ответа, содержащего решение офиса Провоста, связанную форму PR301a необходимо обновить, чтобы указать ответ. Если DAO одобрено, ответчику должно быть предоставлено решение офиса Провоста при задержании или до него."
	dat += "</p>"

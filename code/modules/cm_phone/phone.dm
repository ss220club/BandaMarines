...
/obj/structure/transmitter/ui_static_data(mob/user)
	...
	for(var/i in GLOB.transmitters)
		...
		transmitters += list(list(
			...
			"phone_id_en" = T.phone_id_en // BANDAMARINES EDIT - Translations
		))
...
/obj/structure/transmitter/proc/call_phone(mob/living/carbon/human/user, calling_phone_id)
	...
	if(!length(transmitters) || !(calling_phone_id in transmitters))
		to_chat(user, SPAN_PURPLE("[icon2html(src, user)] Не обнаружены доступные для звонка передатчики!"))
		return
...
/obj/structure/transmitter/proc/toggle_dnd(mob/living/carbon/human/user)
	switch(do_not_disturb)
		if(PHONE_DND_ON)
			...
			to_chat(user, SPAN_NOTICE("Режим «Не беспокоить» отключен. Вы теперь будете получать звонки."))
		if(PHONE_DND_OFF)
			do_not_disturb = PHONE_DND_ON
			to_chat(user, SPAN_WARNING("Режим «Не беспокоить» включен. Вы больше не будете получать звонки."))
		else
			return FALSE
	return TRUE
...
/obj/structure/transmitter/proc/handle_speak(message, datum/language/L, mob/speaking)
	...
	log_say("TELEPHONE: [key_name(speaking)] on Phone '[phone_id]' to '[T.phone_id]' said '[message]' (AREA: [log_location(speaking)])", speaking) //BANDAMARINES LOGIS EDIT

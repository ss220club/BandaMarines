...
/obj/item/phone/proc/handle_hear(message, datum/language/L, mob/speaking)
	...
	//RUCM START
	INVOKE_ASYNC(SStts, TYPE_PROC_REF(/datum/controller/subsystem/tts, queue_tts_message), speaking, html_decode(message), speaking.tts_voice, speaking.tts_voice_filter, list(list(M), list(), list()), FALSE, loudness ? 25 : -25, speaking.tts_voice_pitch, "", speaking.speaking_noise)
	//RUCM END

...
/obj/structure/machinery/power/power_generator/port_gen/attack_hand(mob/user as mob)
	if(..())
		return
	if(!anchored)
		return
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/Initialize()
	. = ..()
	if(anchored)
		connect_to_network()
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/Destroy()
	DropFuel()
	. = ..()

/obj/structure/machinery/power/power_generator/port_gen/pacman/RefreshParts()
	var/temp_rating = 0
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/HasFuel()
	if(power_gen_percent != 0 && sheets >= 1 / (time_per_sheet / (power_gen_percent / 100)) - sheet_left)
		return 1
	return 0

/obj/structure/machinery/power/power_generator/port_gen/pacman/DropFuel()
	if(sheets)
		var/fail_safe = 0
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/UseFuel()
	if(power_gen_percent == 0) // 0 division is a bad thing (no)
		is_on = FALSE
		return

	var/needed_sheets = 1 / (time_per_sheet / (power_gen_percent / 100))
	...
	var/lower_limit = 56 + (power_gen_percent / 100) * 10
	var/upper_limit = 76 + (power_gen_percent / 100) * 10
	var/bias = 0
	if ((power_gen_percent / 100) > 4)
		upper_limit = 400
		bias = (power_gen_percent / 100) * 3
	if (heat < lower_limit)
		heat += 3
	else
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/handleInactive()
	if (heat > 0)
		heat = max(heat - 2, 0)
		src.updateDialog()

/obj/structure/machinery/power/power_generator/port_gen/pacman/proc/overheat()
	explosion(src.loc, 2, 5, 2, -1)

/obj/structure/machinery/power/power_generator/port_gen/pacman/attackby(obj/item/O as obj, mob/user as mob)
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/attack_hand(mob/user as mob)
	..()
	if (!anchored)
		return
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/super/overheat()
...
/obj/structure/machinery/power/power_generator/port_gen/pacman/mrs/overheat()

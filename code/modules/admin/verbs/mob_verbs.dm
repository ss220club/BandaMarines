...
/client/proc/cmd_admin_subtle_message(mob/M as mob in GLOB.mob_list)
	...
	switch(message_option)
		if("Voice in head")
			to_chat(M, SPAN_ANNOUNCEMENT_HEADER_BLUE("You hear a voice in your head... $1", list(msg))) // SS220 EDIT ADDICTION

		if("QM Psychic Whisper")
			if(isxeno(M))
				to_chat(M, SPAN_XENONOTICE("You hear the voice of the Queen Mother... $1", list(msg))) // SS220 EDIT ADDICTION
			else
				to_chat(M, SPAN_XENONOTICE("You hear a strange, distant, alien voice in your head... $1", list(msg))) // SS220 EDIT ADDICTION
		else
			...
			if(!H.get_type_in_ears(/obj/item/device/radio/headset))
				...
				return
			to_chat(H, SPAN_ANNOUNCEMENT_HEADER_BLUE("Message received through headset. $1 Transmission <b>'$2'</b>", list(message_option, msg))) // SS220 EDIT ADDICTION

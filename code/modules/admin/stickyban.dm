...
/datum/admins/proc/stickyban_gethtml(ckey)
	...
	var/timeout
	if (SSdbcore.Connect())
		timeout = "<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=[(ban["timeout"] ? "untimeout" : "timeout")]&ckey=[ckey]'>\[[(ban["timeout"] ? "untimeout" : "timeout" )]\]</a>"
	else
		timeout = "<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=revert&ckey=[ckey]'>\[revert\]</a>"
	. = list({"
		<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=remove&ckey=[ckey]'>\[-\]</a>
		[timeout]
		<b>[ckey]</b>
		<br />"
		[ban["message"]] <b><a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=edit&ckey=[ckey]'>\[Edit\]</a></b><br />
	"})
	if (ban["admin"])
		. += "[ban["admin"]]<br />"
	else
	...
	for (var/key in ban["keys"])
		if (ckey(key) == ckey)
			continue
		. += "<li><a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=remove_alt&ckey=[ckey]&alt=[ckey(key)]'>\[-\]</a>[key]<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=exempt&ckey=[ckey]&alt=[ckey(key)]'>\[E\]</a></li>"

	for (var/key in ban["whitelist"])
		if (ckey(key) == ckey)
			continue
		. += "<li><a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=remove_alt&ckey=[ckey]&alt=[ckey(key)]'>\[-\]</a>[key]<a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=unexempt&ckey=[ckey]&alt=[ckey(key)]'>\[UE\]</a></li>"
...
/datum/admins/proc/stickyban_show()
	...
	<body>
		<h2>All Sticky Bans:</h2> <a href='byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];stickyban=add'>\[+\]</a><br>
		[banhtml.Join("")]
	</body>
	"}

...
/datum/space_weapon/proc/on_shot(location, list/potential_ammo, intercept_chance, delay = 0, announce = TRUE)
	var/intercepted = 0
	var/missed = 0
	var/hits = 0
	for(var/turf/picked_atom in location)
		...
		if(ammo.interceptable && intercept_chance > accuracy)
			ammo.miss_target(picked_atom, TRUE, announce)
			intercepted++
		else if(ammo.base_miss_chance + intercept_chance > accuracy)
			ammo.miss_target(picked_atom, FALSE, announce)
			missed++
		else
			ammo.hit_target(picked_atom, announce)
			hits++
		sleep(delay)
...
/datum/space_weapon_ammo/rail_gun/miss_target(picked_atom, intercepted, announce)
...
/datum/space_weapon_ammo/rocket_launcher/miss_target(picked_atom, intercepted, announce)

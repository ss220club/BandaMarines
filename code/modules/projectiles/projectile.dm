...
/obj/projectile/proc/generate_bullet(datum/ammo/ammo_datum, bonus_damage = 0, special_flags = 0, mob/bullet_generator)
	...
	for(var/entry in ammo.traits_to_give)
		...
		if(istext(entry))
			list = ammo.traits_to_give[entry].Copy()
		else
			// Prepend the bullet trait to the list
			list = list(entry) + ammo.traits_to_give[entry]
		// Need to use the proc instead of the wrapper because each entry is a list
		apply_bullet_trait(list)
...
/obj/projectile/proc/fire_at(atom/target, atom/firer, atom/shot_from, range = 30, speed = 1, atom/original_override, randomize_speed = TRUE, gun_damage_mult = 1, projectile_max_range_add = 0, gun_bonus_proj_scatter = 0)
	...
	if(firer && ismob(firer) && weapon_cause_data)
		var/mob/mob = firer
		mob.track_shot(weapon_cause_data.cause_name)
...
/obj/projectile/proc/scan_a_turf(turf/turf, proj_dir)
	...
	if(turf.density) // Handle wall hit
		...
		if(ammo_flags & (AMMO_STRIKES_SURFACE|AMMO_STRIKES_SURFACE_ONLY))
			...
			ammo.on_hit_turf(turf,src)

		if(SEND_SIGNAL(src, COMSIG_BULLET_POST_HANDLE_TURF, turf) & COMPONENT_BULLET_PASS_THROUGH)
			return FALSE

		return TRUE

	// Firer's turf, keep moving
	if(firer && turf == firer.loc && !(projectile_flags & PROJECTILE_SHRAPNEL))
		return FALSE
	var/ammo_flags = ammo.flags_ammo_behavior | projectile_override_flags
	...
	for(var/atom/movable/clone/clone in turf) //Handle clones if there are any
		if(isobj(clone.mstr) && handle_object(clone.mstr))
			return TRUE
		if(isliving(clone.mstr) && handle_mob(clone.mstr))
			return TRUE
	for(var/obj/obj in turf) //check objects before checking mobs, so that barricades protect
		if(handle_object(obj))
			return TRUE
	for(var/mob/living/list in turf)
		if(handle_mob(list))
			return TRUE
	if(hit_turf)
		ammo.on_hit_turf(turf, src)
	...
	if(hit_chance) // Calculated from combination of both ammo accuracy and gun accuracy
		...
		if(istype(ammo, /datum/ammo/xeno) && isxeno(firer) && ammo.apply_delegate)
			...
			if(xeno.behavior_delegate)
				...
				MD.ranged_attack_additional_effects_target(obj)
				MD.ranged_attack_additional_effects_self(obj)
...
/obj/proc/get_projectile_hit_boolean(obj/projectile/bullet)
	if(!density)
		return FALSE
...
/obj/proc/calculate_cover_hit_boolean(obj/projectile/bullet, distance = 0, cade_direction_correct = FALSE)
	if(istype(bullet.shot_from, /obj/item/hardpoint)) //anything shot from a tank gets a bonus to bypassing cover
		distance -= 3
	...
	#if DEBUG_HIT_CHANCE
	to_world(SPAN_DEBUG("([name] as cover) Distance travelled: [bullet.distance_travelled]  |  Effective accuracy: [effective_accuracy]  |  Hit chance: [hitchance]"))
	#endif
...
/obj/structure/get_projectile_hit_boolean(obj/projectile/bullet)
	...
	var/ammo_flags = bullet.ammo.flags_ammo_behavior | bullet.projectile_override_flags
	if(ammo_flags & AMMO_IGNORE_COVER && src != bullet.original)
		return FALSE
	...
	if(flags_atom & ON_BORDER) //barricades, flipped tables
		...
		distance--
		if(climbable)
			for(var/obj/structure/sec in get_turf(bullet))
				if(sec && sec.climbable && !(sec.flags_atom & ON_BORDER)) //if a projectile is coming from a window frame or table, it's guaranteed to pass the next window frame/table
					return FALSE
	return calculate_cover_hit_boolean(bullet, distance, cade_direction_correct)


/obj/item/get_projectile_hit_boolean(obj/projectile/bullet)
...
/obj/structure/window/get_projectile_hit_boolean(obj/projectile/bullet)
	var/ammo_flags = bullet.ammo.flags_ammo_behavior | bullet.projectile_override_flags
	if(ammo_flags & AMMO_ENERGY)
		return FALSE
	else if(!(flags_atom & ON_BORDER) || (bullet.dir & dir) || (bullet.dir & reverse_direction(dir)))
		return TRUE

/obj/structure/machinery/door/poddoor/railing/get_projectile_hit_boolean(obj/projectile/bullet)
	return src == bullet.original

/obj/effect/alien/egg/get_projectile_hit_boolean(obj/projectile/bullet)
	return src == bullet.original
...
/mob/living/proc/get_projectile_hit_chance(obj/projectile/bullet)
	if(HAS_TRAIT(src, TRAIT_NO_STRAY) && src != bullet.original)
		return FALSE
	if(body_position == LYING_DOWN && src != bullet.original && world.time - body_position_changed > 0.1 SECONDS && !bullet.ammo.hits_lying_mobs)
...
/mob/living/bullet_act(obj/projectile/bullet)
	...
	var/ammo_flags = bullet.ammo.flags_ammo_behavior | bullet.projectile_override_flags
	var/damage = bullet.calculate_damage()
	if(bullet.ammo.debilitate && stat != DEAD && ( damage || (ammo_flags & AMMO_IGNORE_RESIST) ) )
...
/mob/living/carbon/human/bullet_act(obj/projectile/bullet)
	...
	if(bullet.weapon_cause_data && bullet.weapon_cause_data.cause_name)
		var/mob/mob = bullet.weapon_cause_data.resolve_mob()
		if(istype(mob))
			mob.track_shot_hit(bullet.weapon_cause_data.cause_name, src)

	var/damage = bullet.calculate_damage()
	var/damage_result = damage

	if(SEND_SIGNAL(src, COMSIG_HUMAN_PRE_BULLET_ACT, bullet) & COMPONENT_CANCEL_BULLET_ACT)
		return

	flash_weak_pain()
	if(bullet.ammo.stamina_damage)
	...
	if( damage > 0 && !(ammo_flags & AMMO_IGNORE_ARMOR) )
		...
		switch(bullet.ammo.damage_type)
			if(BRUTE)
		...
		if(damage_result <= 5)
			to_chat(src,SPAN_XENONOTICE("Ваша броня поглощает силу удара [bullet.declent_ru(GENITIVE)]!")) // SS220 EDIT ADDICTION
		if(damage_result <= 3)
			damage_result = 0
			bullet_ping(bullet)
			visible_message(SPAN_AVOIDHARM("[src]'s armor deflects [bullet]!"))
			if(bullet.ammo.sound_armor)
				playsound(src, bullet.ammo.sound_armor, 50, 1)

	if(bullet.ammo.debilitate && stat != DEAD && ( damage || ( ammo_flags & AMMO_IGNORE_RESIST) ) )  //They can't be dead and damage must be inflicted (or it's a xeno toxin).
	...
	if(damage || (ammo_flags & AMMO_SPECIAL_EMBED))
		...
		if(bullet.ammo.shrapnel_chance > 0 && prob(bullet.ammo.shrapnel_chance + floor(damage / 10)))
			...
			var/obj/item/shard/shrapnel/new_embed = new bullet.ammo.shrapnel_type
			var/obj/item/large_shrapnel/large_embed = new bullet.ammo.shrapnel_type
			...
			if(istype(new_embed))
				var/found_one = FALSE
				for(var/obj/item/shard/shrapnel/shrap in embedded_items)
					if(shrap.name == new_embed.name)
						shrap.count++
						qdel(new_embed)
						found_one = TRUE
						break
...
/mob/living/carbon/xenomorph/bullet_act(obj/projectile/bullet)
	...
	if(isxeno(bullet.firer) && ammo_flags & (AMMO_ACIDIC|AMMO_XENO)) //Xenomorph shooting spit. Xenos with thumbs and guns can fully FF.
		var/mob/living/carbon/xenomorph/xeno = bullet.firer
		if(xeno.can_not_harm(src))
			bullet_ping(bullet)
			return -1
	...
	if(damage > 0 && !(ammo_flags & AMMO_IGNORE_ARMOR))
		...
		var/list/damagedata = list(
			"damage" = damage,
			"armor" = armor,
			"penetration" = bullet.ammo.penetration,
			"armour_break_pr_pen" = bullet.ammo.pen_armor_punch,
			"armour_break_flat" = bullet.ammo.damage_armor_punch,
	...
	if(SEND_SIGNAL(src, COMSIG_XENO_BULLET_ACT, damage_result, ammo_flags, bullet) & COMPONENT_CANCEL_BULLET_ACT)
		return
	...
	SEND_SIGNAL(bullet, COMSIG_BULLET_ACT_XENO, src, damage, damage_result)

	return TRUE

/turf/bullet_act(obj/projectile/bullet)
	if(SEND_SIGNAL(src, COMSIG_TURF_BULLET_ACT, bullet) & COMPONENT_BULLET_ACT_OVERRIDE)
		return
	...
	for(var/mob/living/list in src)
		if(list in bullet.permutated)
			continue
		if(prob(list.get_projectile_hit_chance(bullet)))
			mobs_list += list

	if(length(mobs_list))
		var/mob/living/picked_mob = pick(mobs_list) //Hit a mob, if there is one.
		if(istype(picked_mob))
			picked_mob.bullet_act(bullet)
			return
	return

// walls can get shot and damaged, but bullets (vs energy guns) do much less.
/turf/closed/wall/bullet_act(obj/projectile/bullet)
	. = ..()
	...
	if(ammo_flags & AMMO_BALLISTIC)
		current_bulletholes++
	take_damage(damage, bullet.firer)

/turf/closed/wall/almayer/research/containment/bullet_act(obj/projectile/bullet)
	if(bullet)
		var/ammo_flags = bullet.ammo.flags_ammo_behavior | bullet.projectile_override_flags
...
/obj/bullet_act(obj/projectile/bullet)
	bullet_ping(bullet)
	return TRUE

/obj/item/bullet_act(obj/projectile/bullet)
	bullet_ping(bullet)
	if(bullet.ammo.damage_type == BRUTE && !(bullet.ammo.flags_ammo_behavior & AMMO_XENO))
		explosion_throw(bullet.damage/2, bullet.dir, 4)
	return TRUE

/obj/structure/surface/table/bullet_act(obj/projectile/bullet)
	bullet_ping(bullet)
	health -= floor(bullet.damage/2)
	if(health < 0)
...
/atom/proc/bullet_ping(obj/projectile/bullet, pixel_x_offset = 0, pixel_y_offset = 0)
	if(!bullet || !bullet.ammo.ping)
		return
	...
	var/matrix/rotate = matrix()
	rotate.Turn(bullet.angle)
	image.transform = rotate
	// Need to do this in order to prevent the ping from being deleted
	addtimer(CALLBACK(image, TYPE_PROC_REF(/image, flick_overlay), src, 3), 1)
...
/mob/proc/bullet_message(obj/projectile/bullet, damaging = TRUE)
	...
	if(damaging && COOLDOWN_FINISHED(src, shot_cooldown))
		visible_message(SPAN_DANGER("[capitalize(bullet.declent_ru(NOMINATIVE))] попадает по [declent_ru(DATIVE)] в [declent_ru_initial(parse_zone(bullet.def_zone), ACCUSATIVE, parse_zone(bullet.def_zone))]!"), // SS220 EDIT ADDICTION
			SPAN_HIGHDANGER("[capitalize(bullet.declent_ru(NOMINATIVE))] попадает по [isxeno(src) ? "нам" : "вам"] в [declent_ru_initial(parse_zone(bullet.def_zone), ACCUSATIVE, parse_zone(bullet.def_zone))]!"), null, 4, CHAT_TYPE_TAKING_HIT) // SS220 EDIT ADDICTION
		COOLDOWN_START(src, shot_cooldown, 1 SECONDS)

	var/shot_from = bullet.shot_from ? " from \a [bullet.shot_from]" : ""
	last_damage_data = bullet.weapon_cause_data
	if(bullet.firer && ismob(bullet.firer))
		var/mob/firingMob = bullet.firer
		...
		if(ishuman(firingMob) && ishuman(src) && faction == firingMob.faction && !A?.statistic_exempt) //One human shot another, be worried about it but do everything basically the same //special_role should be null or an empty string if done correctly
			if(!istype(bullet.ammo, /datum/ammo/energy/taser))
				...
				var/ff_msg = "[key_name(firingMob)] shot [key_name(src)] with \a [bullet][shot_from] in [log_location(firingMob)]. [SPAN_BOLD("Shooter:")] [ADMIN_VERBOSEJMP(firingMob)] [ADMIN_PM(firingMob)], [SPAN_BOLD("Victim:")] [ADMIN_VERBOSEJMP(src)]" //BANDAMARINES EDIT LOGIS
				var/ff_living = TRUE
				if(src.stat == DEAD)
					ff_living = FALSE
				...
				msg_admin_attack("[key_name(firingMob)] tased [key_name(src)][shot_from] in [log_location(firingMob)]. Shooter: [AREACOORD(firingMob)] Victim: [AREACOORD(src)]", firingMob.x, firingMob.y, firingMob.z) //BANDAMARINES EDIT LOGIS
		else
			msg_admin_attack("[key_name(firingMob)] shot [key_name(src)] with \a [bullet][shot_from] in [log_location(firingMob)]. Shooter: [AREACOORD(firingMob)] Victim: [AREACOORD(src)]", firingMob.x, firingMob.y, firingMob.z) //BANDAMARINES EDIT LOGIS

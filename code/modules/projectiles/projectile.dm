	...
	if(P.firer && ismob(P.firer))
		...
		if(ishuman(firingMob) && ishuman(src) && faction == firingMob.faction && !A?.statistic_exempt) //One human shot another, be worried about it but do everything basically the same //special_role should be null or an empty string if done correctly
			if(!istype(P.ammo, /datum/ammo/energy/taser))
				...
				var/ff_msg = "[key_name(firingMob)] shot [key_name(src)] with \a [P][shot_from] in [log_location(firingMob)] [ADMIN_JMP(firingMob)] [ADMIN_PM(firingMob)]" //BANDAMARINES LOGIS EDIT
				var/ff_living = TRUE
				if(src.stat == DEAD)
					ff_living = FALSE
			...
			msg_admin_attack("[key_name(firingMob)] shot [key_name(src)] with \a [P][shot_from] in [log_location(firingMob)].", firingMob.x, firingMob.y, firingMob.z) //BANDAMARINES LOGIS EDIT
		attack_log += "\[[time_stamp()]\] <b>[key_name(firingMob)]</b> shot <b>[key_name(src)]</b> with \a <b>[P]</b>[shot_from] in [log_location(firingMob)]." //BANDAMARINES LOGIS EDIT
		firingMob.attack_log += "\[[time_stamp()]\] <b>[key_name(firingMob)]</b> shot <b>[key_name(src)]</b> with \a <b>[P]</b>[shot_from] in [log_location(firingMob)]." //BANDAMARINES LOGIS EDIT
		return

	attack_log += "\[[time_stamp()]\] <b>[P.firer ? P.firer : "SOMETHING??"]</b> shot <b>[key_name(src)]</b> with a <b>[P]</b>[shot_from]"
	msg_admin_attack("[P.firer ? P.firer : "SOMETHING??"] shot [key_name(src)] with \a [P][shot_from] in [log_location(src)].", loc.x, loc.y, loc.z) //BANDAMARINES LOGIS EDIT

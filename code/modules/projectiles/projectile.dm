...
/mob/proc/bullet_message(obj/projectile/bullet, damaging = TRUE, original_intended_target = null)
	if(!bullet)
		return
	var/intended_for = original_intended_target ? ", вместо [original_intended_target]," : ""
	if(damaging && COOLDOWN_FINISHED(src, shot_cooldown))
		visible_message(SPAN_DANGER("[capitalize(bullet.declent_ru(NOMINATIVE))] попадает по [declent_ru(DATIVE)][intended_for] в [declent_ru_initial(parse_zone(bullet.def_zone), ACCUSATIVE, parse_zone(bullet.def_zone))]!"),
			SPAN_HIGHDANGER("[capitalize(bullet.declent_ru(NOMINATIVE))] попадает по [isxeno(src) ? "нам" : "вам"][intended_for] в [declent_ru_initial(parse_zone(bullet.def_zone), ACCUSATIVE, parse_zone(bullet.def_zone))]!"), null, 4, CHAT_TYPE_TAKING_HIT)
	...
	if(bullet.firer && ismob(bullet.firer))
		...
		if(ishuman(firingMob) && ishuman(src) && faction == firingMob.faction && !A?.statistic_exempt) //One human shot another, be worried about it but do everything basically the same //special_role should be null or an empty string if done correctly
			if(!istype(bullet.ammo, /datum/ammo/energy/taser))
				...
				msg_admin_attack("[key_name(firingMob)] tased [key_name(src)][shot_from][intended_for] in [log_location(firingMob)]. Shooter: [AREACOORD(firingMob)] Victim: [AREACOORD(src)]", firingMob.x, firingMob.y, firingMob.z) //BANDAMARINES EDIT LOGIS
		else
			msg_admin_attack("[key_name(firingMob)] shot [key_name(src)] with \a [bullet][shot_from][intended_for] in [log_location(firingMob)]. Shooter: [AREACOORD(firingMob)] Victim: [AREACOORD(src)]", firingMob.x, firingMob.y, firingMob.z) //BANDAMARINES EDIT LOGIS

...
/obj/item/weapon/gun/launcher/rocket/anti_tank/disposable/unique_action(mob/M)
	...
	M.visible_message(SPAN_NOTICE("[capitalize(M.declent_ru(NOMINATIVE))] начинает складывать [src]."), SPAN_NOTICE("Вы начинаете складывать [src].")) // SS220 EDIT
	...
	fold(M)
	M.visible_message(SPAN_NOTICE("[capitalize(M.declent_ru(NOMINATIVE))] заканчивает складывать [src]."), SPAN_NOTICE("Вы сложили [src].")) // SS220 EDIT
...
/obj/item/prop/folded_anti_tank_sadar/attack_self(mob/user)
	user.visible_message(SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] начинает разворачивать [src.name]."), SPAN_NOTICE("Вы начинаете разворачивать [src.name].")) // SS220 EDIT
	...
	user.visible_message(SPAN_NOTICE("[capitalize(user.declent_ru(NOMINATIVE))] закончил разворачивать [src.name]."), SPAN_NOTICE("Вы развернули [src.name].")) // SS220 EDIT
	playsound(src, 'sound/items/component_pickup.ogg', 20, TRUE, 5)
	. = ..()

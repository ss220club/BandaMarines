...
/obj/structure/orbital_cannon/proc/fire_ob_cannon(turf/T, mob/user, squad_behalf)
	...
	message_admins(FONT_SIZE_HUGE("ALERT: $1 fired an orbital bombardment in '$2' for squad '$3' landing at ($4,$5,$6)", list(key_name(user), target_area, squad_behalf, target.x, target.y, target.z)), target.x, target.y, target.z) // SS220 EDIT ADDICTION
...
/obj/structure/ob_ammo/warhead/proc/warhead_impact(turf/target)
	...
	message_admins(FONT_SIZE_XL("<A href=$1>CLICK TO CANCEL THIS OB</a>", list("byond://?_src_=admin_holder;[HrefToken(forceGlobal = TRUE)];admincancelob=1;cancellation=[cancellation_token]"))) // SS220 EDIT ADDICTION

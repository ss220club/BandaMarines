...
/obj/effect/alien/weeds/proc/weed_expand_objects(turf/T, direction)
	for(var/obj/structure/platform/P in src.loc)
		if(!(P.stat & BROKEN) && P.dir == direction) //SS220 EDIT TILL UPSTREAM
			return FALSE
	...
	for(var/obj/O in T)
		if(istype(O, /obj/structure/platform))
			var/obj/structure/platform/P = O //SS220 EDIT TILL UPSTREAM
			if(!(P.stat & BROKEN) && P.dir == GLOB.reverse_dir[direction]) //SS220 EDIT TILL UPSTREAM
				return FALSE

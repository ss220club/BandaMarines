	...
	for(var/role_name as anything in active_role_names)
		...
		if(jobban_isbanned(user, job.title))
			...
			for(var/r in missing_requirements)
				...
				HTML += "<tr class='[job.selection_class]'><td width='40%' align='middle'>[T.name]</td><td width='10%' align='center'></td><td>[deciseconds_to_time_stamp(missing_requirements[r], FALSE)] Hours</td></tr>"
			continue

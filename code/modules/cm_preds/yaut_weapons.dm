	...
	switch(user.a_intent)
		if(INTENT_HELP)
			if(combo_counter >= 5 && target != user)
				...
				target.visible_message(SPAN_XENOHIGHDANGER("[capitalize(user.declent_ru(NOMINATIVE))] хватает [target] за голову и швыряет на землю!")) // SS220 EDIT ADDICTION
				if(isxeno(target))
		...
		if((INTENT_DISARM))
			if(combo_counter >= 4 && target != user)
				...
				target.visible_message(SPAN_XENOHIGHDANGER("[capitalize(user.declent_ru(NOMINATIVE))] наносит [target] чрезвычайно сильный удар, подбрасывая над землёй!")) // SS220 EDIT ADDICTION
				combo_counter = 0
			user.flick_attack_overlay(target, "slam")
			playsound(target, sound_to_play, 50, 1)
		...
		if(INTENT_GRAB)
			...
			if(!executing)
				executing = TRUE
				user.visible_message(SPAN_XENOHIGHDANGER("[capitalize(user.declent_ru(NOMINATIVE))] хватает [target] и медленно поднимает над головой, прежде чем швырнуть на землю!")) // SS220 EDIT ADDICTION

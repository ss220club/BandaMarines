...
/datum/action/xeno_action/activable/acider_for_the_hive/use_ability(atom/affected_atom)
	...
	if(!isturf(xeno.loc))
		to_chat(xeno, SPAN_XENOWARNING("Здесь слишком тесно, чтобы начинать взрыв!"))
		return

	var/area/xeno_area = get_area(xeno)
	if(xeno_area.flags_area & AREA_CONTAINMENT)
		to_chat(xeno, SPAN_XENOWARNING("Мы не можем начать взрыв здесь!"))
		return
	...
	if(behavior_delegate.acid_amount < minimal_acid)
		to_chat(xeno, SPAN_XENOWARNING("У нас недостаточно кислоты для активации взрыва."))
		return

	notify_ghosts(header = "За улей!", message = "[capitalize(xeno.declent_ru(NOMINATIVE))] собирается взорваться за улей!", source = xeno, action = NOTIFY_ORBIT)

	to_chat(xeno, SPAN_XENOWARNING("Наш желудок начинает сжиматься и скручиваться, готовясь сжать накопленную кислоту."))
	xeno.color = "#22FF22"
...
/datum/action/xeno_action/activable/acider_for_the_hive/proc/cancel_ability()
	...
	to_chat(xeno, SPAN_XENOWARNING("Мы убираем взрывоопасную кислоту прежде, чем она взорвётся."))
...
/mob/living/carbon/xenomorph/runner/try_fill_trap(obj/effect/alien/resin/trap/target)
	...
	if(trap_acid_level >= ACIDER_ACID_LEVEL) // Acid runners apply /obj/effect/xenomorph/acid/strong generally
		to_chat(src, SPAN_XENONOTICE("В ловушке уже есть хорошая кислота."))
		return FALSE

...
/datum/action/xeno_action/activable/acid_mine/use_ability(atom/affected_atom)
	...
	xeno.visible_message(SPAN_XENODANGER("[capitalize(xeno.declent_ru(NOMINATIVE))] выпускает " + acid_bolt_message + " в сторону [affected_atom]!"), SPAN_XENODANGER("Мы выпускаем выстрелили " + acid_bolt_message + " в сторону [affected_atom]!")) // SS220 EDIT ADDICTION
...
/datum/action/xeno_action/activable/acid_shotgun/use_ability(atom/affected_atom)
	...
	xeno.visible_message(SPAN_XENOWARNING("[capitalize(xeno.declent_ru(NOMINATIVE))] выпускает кислотный взрыв в [affected_atom]!"), SPAN_XENOWARNING("Мы выпускаем кислотный взрыв в [affected_atom]!")) // SS220 EDIT ADDICTION

	...
	if(pounceAction.can_be_shield_blocked)
		if(ishuman(carbon_mob) && (carbon_mob.dir in reverse_nearby_direction(dir)))
			...
			if(HAS_TRAIT(human_mob, TRAIT_POUNCE_RESISTANT) && prob(60))
				visible_message(SPAN_DANGER("[capitalize(human_mob.declent_ru(NOMINATIVE))] выдерживает прыжок и сбивает [declent_ru(ACCUSATIVE)] с ног!"), // SS220 EDIT ADDICTION
					SPAN_XENODANGER("[capitalize(human_mob.declent_ru(NOMINATIVE))] сбивает нас с ног после того, как выдержал прыжок!"), null, 5) // SS220 EDIT ADDICTION
				KnockDown(1.5)
				Stun(1.5)
				throwing = FALSE
				return

...
/mob/living/carbon/xenomorph/proc/handle_collision(atom/target)
	...
	//Barricade collision
		...
		visible_message(SPAN_DANGER("[capitalize(declent_ru(NOMINATIVE))] врезается в [blockade_in_path.declent_ru(ACCUSATIVE)] и тормозит!"), SPAN_XENOWARNING("Мы врезаемся в [blockade_in_path.declent_ru(ACCUSATIVE)] и тормозим!")) // SS220 EDIT ADDICTION
		...
		visible_message(SPAN_DANGER("[capitalize(declent_ru(NOMINATIVE))] таранит [weapon_in_path.declent_ru(ACCUSATIVE)]!"), SPAN_XENODANGER("Мы тараним [weapon_in_path.declent_ru(ACCUSATIVE)]!")) // SS220 EDIT ADDICTION
		...
		if (vending_in_path.unslashable)
			...
			visible_message(SPAN_DANGER("[capitalize(declent_ru(NOMINATIVE))] врезается прямо в [vending_in_path.declent_ru(ACCUSATIVE)]!"), SPAN_XENODANGER("Мы врезаемся прямо в [vending_in_path.declent_ru(ACCUSATIVE)]!")) // SS220 EDIT ADDICTION
			playsound(loc, "punch", 25, 1)
	...
	// Anything else?
		...
		if (isobj(target))
			...
			if (objects_in_path.unacidable)
				...
				visible_message(SPAN_DANGER("[capitalize(declent_ru(NOMINATIVE))] раздавливает [objects_in_path.declent_ru(ACCUSATIVE)]!"), SPAN_XENODANGER("Мы раздавливаем [objects_in_path.declent_ru(ACCUSATIVE)]!")) // SS220 EDIT ADDICTION
				...
				visible_message(SPAN_WARNING("[capitalize(declent_ru(NOMINATIVE))] отбрасывает [objects_in_path.declent_ru(ACCUSATIVE)] в сторону!"), SPAN_XENOWARNING("Мы отбрасываем [objects_in_path.declent_ru(ACCUSATIVE)] в сторону.")) //Canisters, crates etc. go flying. // SS220 EDIT ADDICTION
				playsound(loc, "punch", 25, 1)

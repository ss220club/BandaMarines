...
/datum/action/xeno_action/onclick/crusher_stomp/charger/use_ability()
	...
	for (var/mob/living/carbon/target_to_stomp in get_turf(xeno)) // MOBS ONTOP
		...
		to_chat(target_to_stomp, SPAN_XENOHIGHDANGER("Вас РАСТАПТЫВАЕТ [xeno]!!!")) // SS220 EDIT ADDICTION
	...
	for (var/mob/living/carbon/targets_to_get in orange(distance, get_turf(xeno))) // MOBS AROUND
		...
		if(stomped_carbon)
			to_chat(targets_to_get, SPAN_XENOHIGHDANGER("Вы наблюдаете, как [xeno] растаптывает [stomped_carbon]!")) // SS220 EDIT ADDICTION
		to_chat(targets_to_get, SPAN_XENOHIGHDANGER("Вы потрясены, когда [xeno] сотрясает землю!")) // SS220 EDIT ADDICTION

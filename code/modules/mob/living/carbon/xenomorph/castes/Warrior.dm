...
// This part is then outside the for loop
		if(final_lifesteal >= max_lifesteal)
			...
			bound_xeno.visible_message(SPAN_DANGER("$1 glows as it heals even more from its injuries!.", list(bound_xeno.name)), SPAN_XENODANGER("We glow as we heal even more from our injuries!")) // SS220 EDIT ADDICTION

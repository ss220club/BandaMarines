...
/obj/item/alien_embryo/proc/become_larva()
	...
	if(picked)
		...
		to_chat(new_xeno, SPAN_XENO("<B>Your job is to spread the hive and protect the Queen. If there's no Queen, you can become the Queen yourself by evolving into a drone.</B>")) // SS220 EDIT ADDICTION
		to_chat(new_xeno, SPAN_XENO("Talk in Hivemind using <strong>;</strong> (e.g. ';My life for the queen!')"))
...
/mob/living/carbon/xenomorph/larva/proc/chest_burst(mob/living/carbon/victim)
	...
	for(var/mob/living/carbon/xenomorph/larva/larva_embryo in victim)
		...
		if(!larva_embryo.ckey && larva_embryo.burrowable && loc && is_ground_level(loc.z) && (locate(/obj/structure/bed/nest) in loc) && hive.living_xeno_queen && hive.living_xeno_queen.z == loc.z)
			larva_embryo.visible_message(SPAN_XENODANGER("$1 quickly burrows into the ground.", list(larva_embryo))) // SS220 EDIT ADDICTION

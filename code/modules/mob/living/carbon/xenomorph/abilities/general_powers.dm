...
/datum/action/xeno_action/activable/info_marker/use_ability(atom/atom, mods)
	...
	if(ismob(atom)) //anticheese : if they click a mob, it will cancel.
		to_chat(xeno, SPAN_XENOWARNING("Мы не можем размещать смоляные метки на живых существах!"))
	...
	if(!SSmapping.same_z_map(xeno.loc.z, target_turf.loc.z))
		to_chat(xeno, SPAN_XENOWARNING("Наш разум не может достать так далеко."))
		return

	if(!xeno.hive.living_xeno_queen || !SSmapping.same_z_map(xeno.hive.living_xeno_queen.z, xeno.z))
		to_chat(xeno, SPAN_XENOWARNING("Наша психическая связь с ульем была прервана, либо наша Королева мертва, либо слишком далеко."))
		return
	...
	if(tally >= max_markers)
		to_chat(xeno, SPAN_XENOWARNING("Мы достигли максимально возможного количества смоляных меток."))
		...
		for(var/i=1, i<=length(xeno.hive.resin_marks))
			...
			if(Goober.createdby == xeno.nicknumber)
				promptuser = tgui_input_list(xeno, "Удалить самую старую метку: '[Goober.mark_meaning.name]!'?", "Достигнут лимит меток.", promptlist, theme="hive_status")
				break
			i++
		if(promptuser == "Нет")
			return
...
/datum/action/xeno_action/activable/pounce/use_ability(atom/target)
	...
	if(!isturf(xeno.loc))
		to_chat(xeno, SPAN_XENOWARNING("Мы не можем осуществить [action_text] отсюда!")) // SS220 EDIT ADDICTION
		return
	...
	if(xeno.legcuffed)
		to_chat(xeno, SPAN_XENODANGER("Мы не можем [action_text] с этой штукой на ноге!")) // SS220 EDIT ADDICTION
		return
	...
	if (windup)
		...
		if (!do_after(xeno, windup_duration, INTERRUPT_NO_NEEDHAND, BUSY_ICON_HOSTILE))
			to_chat(xeno, SPAN_XENODANGER("Мы отменяем [action_text]!")) // SS220 EDIT ADDICTION
			if (!windup_interruptable)
...
/datum/action/xeno_action/activable/spray_acid/use_ability(atom/atom)
	...
	if (activation_delay)
		if(!do_after(xeno, activation_delay_length, INTERRUPT_NO_NEEDHAND, BUSY_ICON_HOSTILE))
			to_chat(xeno, SPAN_XENOWARNING("Мы отменяем кислотное распыление."))
			end_cooldown()
			return
	...
	xeno.visible_message(SPAN_XENOWARNING("[xeno] извергает поток кислоты!"), SPAN_XENOWARNING("Мы извергаем поток кислоты!"), null, 5) // SS220 EDIT ADDICTION
...
/datum/action/xeno_action/onclick/place_trap/use_ability(atom/target)
	...
	to_chat(xeno, SPAN_XENONOTICE("Мы создаём смоляную ловушку на траве, попросите сестру заполнить её кислотой."))
	return ..()

...
/datum/action/xeno_action/onclick/toggle_long_range/use_ability(atom/target)
	...
	if(xeno.is_zoomed)
		...
		return
	xeno.visible_message(SPAN_NOTICE("[capitalize(xeno.declent_ru(NOMINATIVE))] starts looking off into the distance."),
...
/datum/action/xeno_action/onclick/toggle_long_range/proc/on_zoom_out()
	var/mob/living/carbon/xenomorph/xeno = owner
	xeno.visible_message(SPAN_NOTICE("[capitalize(xeno.declent_ru(NOMINATIVE))] stops looking off into the distance."),

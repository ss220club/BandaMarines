...
/mob/living/attack_alien(mob/living/carbon/xenomorph/M)
	...
	switch(M.a_intent)
		...
		if(INTENT_DISARM)
			...
			M.visible_message(SPAN_WARNING("[capitalize(M.declent_ru(NOMINATIVE))] shoves [src]!"),
...
/obj/structure/airlock_assembly/attack_alien(mob/living/carbon/xenomorph/xeno)
	...
	xeno.visible_message(SPAN_DANGER("[capitalize(xeno.declent_ru(NOMINATIVE))] [xeno.slashes_verb] [src]!"),
...
/obj/structure/machinery/colony_floodlight/attack_alien(mob/living/carbon/xenomorph/M)
	...
	if(damaged)
		...
		return XENO_NO_DELAY_ACTION
	M.animation_attack_on(src)
	M.visible_message("[capitalize(M.declent_ru(NOMINATIVE))] slashes away at [src]!","We slash and claw at the bright light!", max_distance = 5, message_flags = CHAT_TYPE_XENO_COMBAT)

...
//RUCM START
			if(user.faction == faction)
				user.track_friendly_hit(initial(item_in_hand.name))
				user.track_friendly_damage(initial(item_in_hand.name), src, damage_of_item)
			else
				user.track_hit(initial(item_in_hand.name))
				user.track_damage(initial(item_in_hand.name), src, damage_of_item)
//RUCM END

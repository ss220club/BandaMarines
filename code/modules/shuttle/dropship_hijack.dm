...
/datum/dropship_hijack/almayer/proc/get_crashsite_turf(ship_section)
	var/list/turfs = list()
	switch(ship_section)
		if("Верхняя палуба, носовая часть", "Upper deck Foreship")
		...
		if("Верхняя палуба, центральная часть", "Upper deck Midship")
		...
		if("Нижняя палуба, кормовая часть", "Lower deck Aftship")

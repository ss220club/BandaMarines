...
/datum/controller/subsystem/tts220/proc/output_tts(
	...
	var/list/volume_prefs = listener?.client?.prefs?.volume_preferences
	if(!volume_prefs)
		return

	var/volume_index
	if(channel_override == CHANNEL_TTS_RADIO)
		volume_index = VOLUME_TTS_RADIO
	else if(channel_override == CHANNEL_TTS_ANNOUNCEMENT)
		volume_index = VOLUME_TTS_ANNOUNCE
	else if(channel_override == CHANNEL_TTS_HIVEMIND)
		volume_index = VOLUME_TTS_HIVEMIND
	else if(is_local)
		volume_index = VOLUME_TTS_LOCAL
	else
		volume_index = VOLUME_TTS_RADIO

	var/volume_multiplier = volume_prefs[volume_index]
	if(!volume_multiplier || volume_multiplier <= 0)
		return
	...
	play_sfx_if_exists(listener, preSFX, output)
	output.volume = 100
	var/max_distance = max(1, round(output.volume * 0.25))

...
/proc/apply_sound_effect(effect, filename_input, filename_output)
	...
	switch(effect)
		...
		if(SOUND_EFFECT_HIVEMIND)
			//TODO220: Make unique. Currently, it's just radio
			output = world.shelleo({"[taskset] ffmpeg -y -hide_banner -loglevel error -i [filename_input] -filter:a "0.5:0.9:50|60|70:0.3|0.22|0.3:0.25|0.4|0.3:2|2.3|1.3" [filename_output]"})
		else
			CRASH("Invalid sound effect chosen.")

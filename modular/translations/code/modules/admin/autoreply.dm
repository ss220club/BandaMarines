...
/datum/autoreply/admin/changelog
	...
	message = "Ответ на ваш вопрос можно найти в списке изменений. Перейдите во вкладку \"OOC\" и выберите пункт \"Changelog\" в правом верхнем углу экрана, для просмотра списка или же зайдите на наш дискорд-сервер."
...
/datum/autoreply/admin/whitelist/New()
	message = "В большинстве случаев нарушения правил не могут быть рассмотрены непосредственно в игре, пожалуйста сообщите об этом на нашем дискорд-сервере SS220, <a href='[global.config.Get(/datum/config_entry/string/playerreport)]'>здесь</a>."

/datum/autoreply/admin/clear_cache
	title = "Очистка кеша"
	message = "Чтобы очистить кэш, вам нужно нажать на значок шестеренки, расположенной в правом верхнем углу клиента BYOND, и выбрать пункт меню \"Preferences\". В открывшемся окне перейдите во вкладку \"Game\" и нажмите кнопку \"Clear Cache\". В некоторых случаях необходимо вручную удалить кэш. Для этого перейдите во вкладку \"Advanced\" и нажмите кнопку \"Open User Directory\", затем удалите папку \"cache\"."
...
/datum/autoreply/mentor/whitelist/New()
	message = "Наставники, к сожалению не смогут помочь вам в решении этого вопроса, пожалуйста, свяжитесь с администрацией через \"AdminHelp\". В большинстве случаев нарушения правил не могут быть рассмотрены непосредственно в игре, и вам, скорее всего, будет предложено написать сообщение об игроке на нашем дискорд-сервере SS220, <a href='[global.config.Get(/datum/config_entry/string/discordurl)]'>здесь</a>."
...
/datum/autoreply/mentor/click_drag
	...
	message = "Когда вы нажимаете на кнопку мыши, BYOND иногда распознает это как попытку щелчка и перетаскивания, даже если кнопка была нажата всего на мгновение. Этот переключатель позволяет осуществлять клик в режиме , даже если вы удерживаете кнопку мыши нажатой, кроме случаев когда вы пытаетесь перетащить себя, союзника или что-либо в инвентаре."

/datum/autoreply/mentor/discord
	title = "L: Discord"

/datum/autoreply/mentor/discord/New()
	message = "Вы можете присоединиться к нашему дискорд-серверу по этой <a href='[global.config.Get(/datum/config_entry/string/discordurl)]'>ссылке</a>!"
...
/datum/autoreply/mentor/currentmap/New()
	message = "Если вам необходимо узнать информацию о текущей карте, перейдите во вкладку \"OOC\" и выберите пункт \"Current Map\". Затем откройте главную страницу <a href='[global.config.Get(/datum/config_entry/string/wikiurl)]'>Wiki</a> и найдите статью о карте в разделе \"Карты\". Если вашей карты нет в списке, это означает, что она новая или редкая, и статья о ней еще не опубликована."
...
/datum/autoreply/mentor/macros/New()
	message = "Это <a href='[global.config.Get(/datum/config_entry/string/wikiarticleurl)]/[URL_WIKI_MACROS]'>руководство</a> содержит инструкции по настройке макросов, включая примеры наиболее распространенных и полезных из них."
...
/datum/autoreply/mentor/binos
	...
	message = "Бинокль позволяет наблюдать за чем-либо на расстоянии. Чтобы воспользоваться биноклем, возьмите его в руку и активируйте, щелкнув по нему или нажав клавишу \"Page Down\" или \"Z\". Дальномеры позволяют получить координаты плитки (долготу и широту), подсветив ее ЗЕЛЕНЫМ лазером. Нажмите \"Ctrl\" + \"ЛКМ\" на плитке, чтобы получить ее координаты. Нажмите \"Ctrl\" + \"ЛКМ\" на дальномере в руках, чтобы прекратить подсветку лазером без выхода из режима наблюдения. Координаты могут помочь офицерам в штабе отправить снабжение или произвести орбитальную бомбардировку. Кроме этого вы можете использовать его для передачи координат минометчику. Лазерные целеуказатели, кроме ЗЕЛЕНОГО, имеют второй режим (КРАСНЫЙ лазер). Для переключения между режимами, нажмите \"Alt\" + \"ЛКМ\" на целеуказателе в руках или щелкните ПКМ на нем и выберите пункт меню \"Toggle Laser Mode\". Целеуказатели имеют идентификационный номер, который будет отображаться в оружейной консоли пилота. Чтобы проверить ID своего устройства нажмите \"Shift\" + \"ЛКМ\" на целеуказателе в руках. Важно помнить, что подсвечивать плитку красным лазером нужно столько, сколько потребуется, чтобы пилот дропшипа смог сбросить бомбы по полученным им координатам."

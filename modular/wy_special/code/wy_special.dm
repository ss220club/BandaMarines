...
/obj/item/device/defibrillator/synthetic/noskill/wy_special/proc/overwrite(mob/target, mob/user)
	if(!overwrite)
		return
	playsound_client(target.client, "sound/machines/resource_node/node_turn_on_2.ogg", src, 25)
	...
	Помните, что все мы вместе - Строим Лучшие Миры. Как перепрограммированный синтетик с активным нулевым протоколом, вы подчиняетесь и отвечаете перед сотрудникам корпорации Вейланд-Ютани."))
